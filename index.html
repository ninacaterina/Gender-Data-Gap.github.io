...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Receipt preview — monospaced to match printed output */
  .receipt-preview {
    background: var(--white);
    border: 1px solid var(--gray-light);
    border-radius: var(--radius);
...
<main>
  ...
  <div class="screen" id="screen-category">
    ...
    <p class="screen-desc">Select one area where the female data gap shows.</p>
  ...
  <div class="screen" id="screen-receipt">
    <p class="screen-label">Your receipt</p>
  ...
  <div class="screen" id="screen-done">
    ...
    <p class="screen-desc" style="margin-bottom: 36px;">The female data gap is real. Your awareness is the first step.</p>
...
const categories = [
  {
    ...
    signal: 'Training load, life phase data',
    fact: `Out of 5,261 sport and exercise science studies, women represent only 34% of total participants. Only 6% of studies focus on females exclusively. For women in midlife and beyond, the data gap is even more pronounced and remains largely undocumented.`
  },
  {
    id: 'performance',
    title: 'Performance\nConditions',
    shortTitle: 'Performance Conditions',
    signal: 'Heat response, exertion data',
  ...
  },
  {
    ...
    signal: 'Reproductive access, legal context',
  ...
  },
  {
    ...
    signal: 'Drug trial representation, dosage data',
  ...
  },
  {
    ...
    signal: 'Health gap economic impact',
    fact: `By 2040, the women's health gap is projected to cause around 75 million years of healthy life lost per year. Closing this gap could boost the global economy by up to one trillion dollars annually and bring an estimated 137 million more women into employment. The female data gap is not only a health issue. It is an economic one.`
  },
  {
    id: 'gyn',
    title: 'Gynecological\nDiseases',
    shortTitle: 'Gynecological Diseases',
    signal: 'Menstrual pain, diagnostic delay',
    fact: `67% of women in Germany report suffering from menstrual pain. Conditions such as endometriosis and PCOS can take an average of ten years to receive a final diagnosis — years marked by dismissed symptoms, inconclusive tests, and the quiet suggestion that the pain might not be real. The delay is not a medical mystery. It is a consequence of decades of under-research.`
  }
];
...
// ── Build receipt text (shared for preview + ESC/POS) ─────────────────────
function buildReceiptLines() {
  ...
  lines.push('FEMALE DATA LAYER');
  lines.push('Default System Receipt');
  lines.push(DIV);
  ...
  lines.push(`Signal: ${cat.signal}`);
  lines.push(`System area: ${cat.shortTitle}`);
  lines.push(DIV);

  // PROCESSING block
  lines.push('');
  lines.push('PROCESSING...');
  lines.push('Reference model: male default');
  lines.push('Female-specific context: unavailable');
  lines.push('Longitudinal data: unavailable');
  lines.push('Hormonal context: not included');
  lines.push('Symptom history: not included');
  lines.push(DIV);

  // OUTPUT block
  lines.push('');
  lines.push('OUTPUT:');
  lines.push('probably psychological');
  lines.push('try again later');
  lines.push('not representative');
  lines.push(DIV);

  // STATUS
  lines.push('');
  lines.push('STATUS:');
  lines.push('*** INVALID RECEIPT ***');
  lines.push(DIV);

  // REASON
  lines.push('');
  lines.push('REASON:');
  lines.push('No evidence was generated.');
  lines.push('Only absence was processed.');
  lines.push(DIV);

  // DATA POINT
  lines.push('');
  lines.push('DATA POINT:');
  const factLines = wrap(cat.fact, W);
  for (const l of factLines) lines.push(l);
  lines.push(DIV);

  // Footer
  lines.push('');
  lines.push('RETURN RECOMMENDED.');
  lines.push(DIV);
  lines.push('');
  lines.push(date);
  lines.push('female-data-layer.github.io');
...
function buildReceiptPreview() {
  ...
  document.getElementById('receipt-preview').textContent = lines.join('\n');
}
...
function buildEscPos() {
  ...
  // Lines that should print bold
  const boldLines = new Set([
    'FEMALE DATA LAYER', 'INPUT:', 'PROCESSING...',
    'OUTPUT:', 'STATUS:', '*** INVALID RECEIPT ***',
    'REASON:', 'DATA POINT:', 'RETURN RECOMMENDED.'
  ]);

  const chunks = [new Uint8Array(ESC_INIT)];

  // Header centred
  chunks.push(new Uint8Array(ALIGN_C));
  chunks.push(new Uint8Array(BOLD_ON));
  chunks.push(enc.encode('FEMALE DATA LAYER\n'));
  chunks.push(new Uint8Array(BOLD_OFF));
  chunks.push(enc.encode('Default System Receipt\n'));
  chunks.push(new Uint8Array(ALIGN_L));

  // Rest of lines
  for (const line of lines.slice(2)) {
    if (boldLines.has(line)) {
      chunks.push(new Uint8Array(BOLD_ON));
      chunks.push(enc.encode(line + '\n'));
      chunks.push(new Uint8Array(BOLD_OFF));
    } else if (line === '*** INVALID RECEIPT ***') {
      chunks.push(new Uint8Array(ALIGN_C));
      chunks.push(new Uint8Array(BOLD_ON));
      chunks.push(enc.encode(line + '\n'));
      chunks.push(new Uint8Array(BOLD_OFF));
      chunks.push(new Uint8Array(ALIGN_L));
    } else if (line === 'female-data-layer.github.io' || /^\d{2}\.\d{2}\.\d{4}$/.test(line)) {
      chunks.push(new Uint8Array(ALIGN_C));
      chunks.push(enc.encode(line + '\n'));
      chunks.push(new Uint8Array(ALIGN_L));
    } else {
